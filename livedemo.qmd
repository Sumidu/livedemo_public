...
responses <- data %>%
  ...
  filter(age < 100) %>%
  select(-starts_with("Fragenzeit.")) %>%
  select(-starts_with("Gruppenzeit.")) %>% 
  noop()
names(responses)
```
...
keys.list <- list(ATI = c("ATI_SQ001", "ATI_SQ002", "-ATI_SQ003",
                          "ATI_SQ004", "ATI_SQ005", "-ATI_SQ006",
                          "ATI_SQ007", "-ATI_SQ008", "ATI_SQ009"))
...
codebook <- tibble(name = names(data)[1:93], label = c(attributes(data)$variable.labels, "Gesamtzeit"))
...
semantic_variables <- responses %>%
  select(id, gender, starts_with(variable_prefix)) %>%
  setNames(c("id", "gender", semantic_labels$left)) %>%
  pivot_longer(cols = c(-id,-gender)) %>%
  mutate(value = as.numeric(value) - (differential_range + 1)) %>%
  group_by(name) %>%
